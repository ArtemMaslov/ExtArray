...
ProgramStatus UtilLib::ExtArrayConstructor(ExtArray* const extArray, const size_t elemSize, const size_t capacity, const float resize)
{
	assert(extArray);
	assert(elemSize > 0);
	assert(capacity > 0);
...
void UtilLib::ExtArrayDestructor(ExtArray* const extArray,
...
ProgramStatus UtilLib::ExtArrayAddElem(ExtArray* const extArray, const void* const elem)
{
	assert(extArray);
	assert(elem);
...
void* UtilLib::ExtArrayGetElemAt(const ExtArray* const extArray, const size_t index)
{
	assert(extArray);
...
static ProgramStatus UtilLib::ExtArrayResize(ExtArray* const extArray)
{
	assert(extArray);
...
#undef ExtArrayDump
void UtilLib::ExtArrayDump(const ExtArray* const extArray,
				  const char* const funct, const char* const file, const size_t line)
{
	assert(extArray);
	assert(funct);
